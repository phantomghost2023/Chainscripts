...
def test_adaptive_execution():
    ...
    for scenario in test_scenarios:
        ...
        execution_time = engine.execute_script(scenario['script'], test_data="benchmark")
        ...
        print(f"  📏 Script Profile: {profile.get('size_category', 'unknown')} ({profile.get('line_count', 'N/A')} lines)")
    ...
    medium_lightweight_scripts = [r for r in results if r['profile'].get('size_category') == 'medium-lightweight']
    medium_balanced_scripts = [r for r in results if r['profile'].get('size_category') == 'medium-balanced']
    ...
    if medium_lightweight_scripts:
        avg_medium_lightweight = sum(r['execution_time'] for r in medium_lightweight_scripts) / len(medium_lightweight_scripts)
        print(f"  📏 Medium-Lightweight Scripts Avg: {avg_medium_lightweight:.4f}s (lightweight optimization)")
    
    if medium_balanced_scripts:
        avg_medium_balanced = sum(r['execution_time'] for r in medium_balanced_scripts) / len(medium_balanced_scripts)
        print(f"  📏 Medium-Balanced Scripts Avg: {avg_medium_balanced:.4f}s (balanced optimization)")
...
def simulate_before_after_comparison():
    ...
    for scenario in scenarios:
        improvement_pct = ((scenario['before'] - scenario['after']) / scenario['before']) * 100
