...
class PredictiveCacheManager:
    ...
    def _predict_and_preload(self, accessed_key: str):
        ...
        try:
            ...
            for i, prob in enumerate(predictions):
                if prob > threshold:
                    ...
                    if "key_encoder" in self.feature_encoders:
                        key = self.feature_encoders["key_encoder"].inverse_transform(
                            [i]
                        )[0]
            ...
            for key, prob in likely_keys[:3]:  # Preload top 3
                threading.Thread(
                    target=self._preload_item, args=(key,), daemon=True
                ).start()
    ...
    def _extract_features(self, key: str) -> List[float]:
        ...
        if key in self.access_patterns:
            recent_accesses = [
                t for t in self.access_patterns[key] if current_time - t < 3600
            ]  # Last hour
            features.extend(
                [
                    len(recent_accesses),
                    ...
                    (
                        current_time - max(self.access_patterns[key])
    ...
    def _extract_features_for_timestamp(
    ...
    def _cleanup_expired(self):
        ...
        expired_keys = [
            key for key, entry in self.cache_index.items() if entry.is_expired()
        ]
    ...
    def _save_cache_index(self):
        ...
        try:
            with open(index_file, "w") as f:
                json.dump(
                    {key: entry.to_dict() for key, entry in self.cache_index.items()}, f
                )
        except Exception as e:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_access_count": sum(
                e.access_count for e in self.cache_index.values()
            )
            / max(len(self.cache_index), 1),
        }
