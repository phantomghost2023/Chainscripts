...
class ThresholdOptimizer:
    ...
    def _analyze_script_metrics(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            patterns = {
                ...
                "has_loops": (node_counts.get("For", 0) + node_counts.get("While", 0))
                > 0,
                "has_comprehensions": any(
                    node_counts.get(t, 0) > 0
                    for t in ["ListComp", "DictComp", "SetComp"]
                ),
                "has_imports": node_counts.get("Import", 0)
                + node_counts.get("ImportFrom", 0)
                > 3,
                "has_decorators": any(
                    "decorator" in str(node).lower() for node in ast.walk(tree)
                ),
            }
            ...
            return {
                ...
                "import_count": node_counts.get("Import", 0)
                + node_counts.get("ImportFrom", 0),
            }
    ...
    def classify_with_current_thresholds(self, metrics: Dict[str, Any]) -> str:
        ...
        if (
            lines <= self.current_thresholds["small"]["lines"]
            and complexity <= self.current_thresholds["small"]["complexity"]
            and nodes <= self.current_thresholds["small"]["ast_nodes"]
        ):
            return "small"
        elif (
            lines <= self.current_thresholds["medium-lightweight"]["lines"]
            and complexity
            <= self.current_thresholds["medium-lightweight"]["complexity"]
            and nodes <= self.current_thresholds["medium-lightweight"]["ast_nodes"]
        ):
            return "medium-lightweight"
        elif (
            lines <= self.current_thresholds["medium-balanced"]["lines"]
            and complexity <= self.current_thresholds["medium-balanced"]["complexity"]
            and nodes <= self.current_thresholds["medium-balanced"]["ast_nodes"]
        ):
    ...
    def suggest_optimal_thresholds(self):
        ...
        for category, metrics_list in categories.items():
            ...
            print(f"   Lines range: {min(lines_values)}-{max(lines_values)}")
            print(f"   (suggested max: {optimal_thresholds[category]['lines']})")
            print(f"   Complexity range: {min(complexity_values)}-{max(complexity_values)}")
            print(f"   (suggested max: {optimal_thresholds[category]['complexity']})")
            print(f"   AST nodes range: {min(nodes_values)}-{max(nodes_values)}")
            print(f"   (suggested max: {optimal_thresholds[category]['ast_nodes']})")
        ...
        config = {
            "classification_thresholds": {
                "small_script": {
                    ...
                    "max_complexity": optimal_thresholds.get("small", {}).get(
                        "complexity", 12
                    ),
                    "max_ast_nodes": optimal_thresholds.get("small", {}).get(
                        "ast_nodes", 120
                    ),
                },
                "medium_lightweight": {
                    "max_lines": optimal_thresholds.get("medium_lightweight", {}).get(
                        "lines", 180
                    ),
                    "max_complexity": optimal_thresholds.get(
                        "medium_lightweight", {}
                    ).get("complexity", 30),
                    "max_ast_nodes": optimal_thresholds.get(
                        "medium_lightweight", {}
                    ).get("ast_nodes", 360),
                },
                "medium_balanced": {
                    "max_lines": optimal_thresholds.get("medium_balanced", {}).get(
                        "lines", 360
                    ),
                    "max_complexity": optimal_thresholds.get("medium_balanced", {}).get(
                        "complexity", 60
                    ),
                    "max_ast_nodes": optimal_thresholds.get("medium_balanced", {}).get(
                        "ast_nodes", 720
                    ),
                },
                "large_script": {
